...
public class Parser {
	...
	void A() {
		while (!(la.kind == 0 || la.kind == 5 || la.kind == 7)) {SynErr(12); Get();}
		if (la.kind == 5) {
			Get();
			Expect(6);
		}
	...
	bool[,] set = {
		{T,x,T,T, T,T,x,T, x,x,x,x},
		{T,x,T,T, T,x,x,x, x,x,x,x}
  ...
  public string errMsgFormat = "-- line {0} col {1}: {2}"; // 0=line, 1=column, 2=text
	...
	public void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 11: s = "this symbol not expected in Test"; break;
			case 12: s = "this symbol not expected in A"; break;
