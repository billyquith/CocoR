...
public class Scanner {
	...
	void Init() {
		...
		for (int i = 65; i <= 90; ++i) start[i] = 12;
		for (int i = 98; i <= 122; ++i) start[i] = 12;
		for (int i = 48; i <= 57; ++i) start[i] = 13;
		start[0] = 9; 
		start[97] = 18; 
		start[Buffer.EOF] = -1;
	...
	void CheckLiteral() {
		switch (t.val) {
			case "abc": t.kind = 7; break;
			case "a": t.kind = 9; break;
			default: break;
		}
	}
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 9:
				{t.kind = 5; break;}
			case 10:
				if (ch == 'c') {AddCh(); goto case 11;}
				else {t.kind = noSym; break;}
			case 11:
				{t.kind = 6; break;}
			case 12:
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'z') {apx = 0; AddCh(); goto case 1;}
				else if (ch == '*') {apx = 0; AddCh(); goto case 2;}
				else if (ch == '_') {apx++; AddCh(); goto case 14;}
				else if (ch == '+') {apx++; AddCh(); goto case 3;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 13:
				if (ch >= '0' && ch <= '9') {AddCh(); goto case 13;}
				else if (ch == '.') {apx++; AddCh(); goto case 15;}
				else {t.kind = 4; break;}
			case 14:
				if (ch == '*') {apx = 0; AddCh(); goto case 2;}
				else if (ch == '_') {apx++; AddCh(); goto case 14;}
				else if (ch == '+') {apx++; AddCh(); goto case 3;}
				else {t.kind = noSym; break;}
			case 15:
				if (ch >= '0' && ch <= '9') {apx = 0; AddCh(); goto case 4;}
				else if (ch == 'E') {apx = 0; AddCh(); goto case 5;}
			...
			case 17:
				{t.kind = 11; break;}
			case 18:
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == 'a' || ch >= 'c' && ch <= 'z') {apx = 0; AddCh(); goto case 1;}
				else if (ch == '*') {apx = 0; AddCh(); goto case 2;}
				else if (ch == '_') {apx++; AddCh(); goto case 19;}
				else if (ch == '+') {apx++; AddCh(); goto case 3;}
				else if (ch == 'b') {apx = 0; AddCh(); goto case 20;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 19:
				if (ch == '*') {apx = 0; AddCh(); goto case 2;}
				else if (ch == '_') {apx++; AddCh(); goto case 21;}
				else if (ch == '+') {apx++; AddCh(); goto case 3;}
				else {t.kind = 10; break;}
			case 20:
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'b' || ch >= 'd' && ch <= 'z') {AddCh(); goto case 1;}
				else if (ch == 0) {AddCh(); goto case 10;}
				else if (ch == 'c') {AddCh(); goto case 22;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 21:
				if (ch == '*') {apx = 0; AddCh(); goto case 23;}
				else if (ch == '_') {apx++; AddCh(); goto case 14;}
				else if (ch == '+') {apx++; AddCh(); goto case 3;}
				else {t.kind = noSym; break;}
			case 22:
